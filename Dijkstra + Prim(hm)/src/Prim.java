...
public class Prim<Vertex> {
    ...
    private void shortestPaths(NodeVertex current, Graph<Vertex> graph, int count) {
        ...
        for (WeightedEdge<Vertex> edge : graph.neighbors(current.vertex)) {
            ...
            if (towardNode == null || !towardNode.marked) {
                if (vertices.containsKey(toward) && + edge.weight() < towardNode.distance) {
                    pq.changePriority(towardNode.vertex, edge.weight());
                    towardNode.distance = edge.weight();
                    towardNode.edgeTo = current.vertex;
                } else if (!vertices.containsKey(toward)) {
                    vertices.put(toward, new NodeVertex(toward, start, edge.weight(), false));
                    towardNode = vertices.get(toward);
                    pq.add(towardNode.vertex, edge.weight());
                }
            }
        }
        if (pq.size() == 0 || count > 1000) {
            return;
        }
    ...
    private void printTree(NodeVertex current, String spaces) {
        ...
        System.out.println(spaces + current.vertex);
        if (branches.size() == 0) {
            return;
        }
